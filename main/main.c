                ...
                for (int i = 0; i < ret_num; i += SOC_ADC_DIGI_RESULT_BYTES) {
                    ...
                    if (check_valid_data(p)) {
                        if(p->type1.channel==6){
                            ...
                            //float filtered_raw = ((float) ( ( prev_value * 96 )  + (raw_data * 4) ) );
                            float avg_data = (float) ( ( prev_value +raw_data ) ) ;

                            double real_hot = ((double)avg_data-870.00)/10.00;
                            #if EXAMPLE_ADC_USE_OUTPUT_TYPE1
                            ESP_LOGI(TAG, "Unit: %d, Channel: %d, Value: %.2f", 1, p->type1.channel,real_hot);
